...
def run_thread(round, train_id, test_id, Ucollection, Lcollection, saver, examples, labels, global_parameters):
    ...
    while len(Ucollection) > 10:
        select_index = qs.select(Ucollection, model=reg)
...
mt = aceThreading(X, y, Train_idx, Test_idx, [IndexCollection(i) for i in U_pool], [IndexCollection(i) for i in L_pool], refresh_interval=0.1)
mt.start_all_threads(run_thread)
